...
import { normalizeRRMMode } from '../types/rrm.ts';
...
const EDITABLE_FIELDS: Record<AppNode['type'], ReadonlySet<string>> = {
  ...
  rrm_reasoning: new Set(['label', 'mode', 'symbol'])
};
...
function parseValue(field: string, rawValue: string): unknown {
  ...
  if (field === 'mode') {
    const mode = normalizeRRMMode(rawValue);
    if (!mode) throw new Error('Mode RRM tidak dikenal. Lihat pilihan mode pada RRM Node.');
    return mode;
  }
