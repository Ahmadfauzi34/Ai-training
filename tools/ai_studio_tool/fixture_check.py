...
def test_f31_memory_topology_betti():
    ...
    # Load memory_graph and memory_analyzers
    mg_spec = importlib.util.spec_from_file_location("memory_graph", str(ROOT / "memory_graph.py"))
    mg_mod = importlib.util.module_from_spec(mg_spec)
    mg_spec.loader.exec_module(mg_mod)

    ma_spec = importlib.util.spec_from_file_location("memory_analyzers", str(ROOT / "memory_analyzers.py"))
    ma_mod = importlib.util.module_from_spec(ma_spec)
    ma_spec.loader.exec_module(ma_mod)
    ...
    graph = mg_mod.build_memory_graph_from_data(memories, associations)
    ...
    breakdown = ma_mod.analyze_betti_breakdown(graph)
