# README.md — Updated for HoTT Kernel Architecture (4.0.0-memory)

```markdown
# AI Studio Codebase Harness & HoTT Kernel

Tool Python ini berfungsi sebagai **harness analisis codebase & memory topology terpadu** untuk AI Studio & LLM Agent (`schema_version: 4.0.0-memory`). Melalui **satu entry point** (`hott_kernel.py`), tool ini menjalankan:
1. **Codebase Intelligence**: 12 analyzer terintegrasi (4 topo, 4 perf, 4 hott) dalam 1 filesystem scan (< 50ms overhead).
2. **Memory Domain**: Persistent episodic & semantic memory, topological memory analyzers (β₀, β₁ breakdown, decay, hub), dan colimit consolidation.
3. **Cross-Domain Bridge**: Unified steering (`xsteer`), auto-store findings (`xanalyze`), dan memory-augmented context (`xcontext`).
...
| **Memory Store & Graph** | Persistent JSON store, memory graph builder dengan edge-type annotations |
| **Memory Synthesizer** | Memory baseline, memory drift detection, memory steering signals |
| **Cross-Domain Bridge** | Unified codebase + memory steering, auto-store, memory-augmented context |

### 2. Analyzer Suite (12 Codebase + 6 Memory)
...
| Prefix | Analyzer | Fungsi |
|---|---|---|
| `mem.fragmentation` | Knowledge Silos (β₀) | Deteksi memori terisolasi dan silo |
| `mem.circular` | Circular Reasoning (β₁) | Edge-type-aware cycle detection (reasoning vs structural) |
| `mem.decay` | Stale Memories | Deteksi memori yang sudah lama tidak diakses |
| `mem.hub` | Memory Hubs | Deteksi memori dengan banyak relasi |
| `mem.manifold` | Memory Manifold | Betti numbers, memory archetype, complexity |
| `mem.betti_breakdown` | Betti Breakdown | Analisis β₁ total vs reasoning vs structural |
...
**Gunakan `hott_kernel.py` untuk semua analisis & memory operations.**
...
#### B. Memory Domain Operations
```bash
# Analisis topologi memori
python3 tools/ai_studio_tool/hott_kernel.py memory analyze
python3 tools/ai_studio_tool/hott_kernel.py memory stats

# Recall dan store memori
python3 tools/ai_studio_tool/hott_kernel.py memory store episodic "Observation text" --source "analyzer" --importance 0.8
python3 tools/ai_studio_tool/hott_kernel.py memory recall "query text"

# Asosiasi & konsolidasi colimit
python3 tools/ai_studio_tool/hott_kernel.py memory associate id1 id2 inferential --strength 0.8
python3 tools/ai_studio_tool/hott_kernel.py memory consolidate id1,id2,id3 --content "Consolidated semantic knowledge"

# Memory baseline, drift & steering
python3 tools/ai_studio_tool/hott_kernel.py memory establish
python3 tools/ai_studio_tool/hott_kernel.py memory drift
python3 tools/ai_studio_tool/hott_kernel.py memory steer --output full
```

#### C. Cross-Domain Operations
```bash
# Codebase analysis dengan auto-store ke memory
python3 tools/ai_studio_tool/hott_kernel.py xanalyze src --output summary

# Unified codebase + memory steering
python3 tools/ai_studio_tool/hott_kernel.py xsteer src --output full

# Context memory untuk file spesifik
python3 tools/ai_studio_tool/hott_kernel.py xcontext src/app/app.ts
```

### 2. Uji Baseline Fixtures (28 Test Cases)

```bash
python3 tools/ai_studio_tool/fixture_check.py
```

Expected output: `PASS: 28 fixture minimal baseline aman`
